...
def cmpl-docker-network-driver [] {
    [bridge host none overlay ipvlan macvlan]
}

export def containers-network-create [
    name: string
    --driver(-d): string@cmpl-docker-network-driver = "bridge"
] {
...
def cmpl-docker-cp [cmd: string, offset: int] {
...
# remove container
export def container-remove [container: string@cmpl-docker-containers -n: string@cmpl-docker-ns] {
...
def cmpl-docker-run-sshkey [ctx: string, pos: int] {
    ls ~/.ssh/**/*.pub
    | get name
    | path relative-to ~/.ssh
}

def cmpl-docker-run-proxy [] {
