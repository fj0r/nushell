...
export def --env dp [container?: string, --all(-a)] {
    $env.config.hooks.pre_execution ++= [ { self-destruct: docker code: 'print $"(ansi grey)load (ansi xterm_grey58)docker(ansi grey)...(ansi reset)";use docker *; $env.config.hooks.pre_execution = $env.config.hooks.pre_execution | where {|x| ($x | describe -d).type != record or $x.self-destruct? != docker }' } ]; print $'(ansi grey)the next command will load the `(ansi xterm_grey58)docker(ansi grey)` module.(ansi grey)'
}
