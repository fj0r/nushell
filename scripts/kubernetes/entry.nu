...
export def --env kn [namespace: string] {
    $env.config.hooks.pre_execution = [ { code: 'print $"(ansi grey)load kubernetes...(ansi reset)";use kubernetes *; $env.config.hooks.pre_execution = ($env.config.hooks.pre_execution | slice ..-2)' } ]
